...
const isValidMercadoLibreProductUrl = (url: string) => {
  try {
    ...
    // check if hostname contains mercadolibre.com.mx or mercadolibre.ca
  } catch (error) {
    return;
  }
};

const ScrapeMercadoLibreSearchBar = () => {
  ...
  const handleSearchSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const isValidLink = isValidMercadoLibreProductUrl(searchPrompt);
    if (!isValidLink)
      return toast({
        title: "Invalid Link",
        description: "Add a valid Mercado Libre product link",
        variant: "destructive",
      });
    ...
    try {
      ...
      console.log(error, "error 89791968");
    } finally {
      setIsLoading(false);
    }
    if (isValidLink) {
...
export default ScrapeMercadoLibreSearchBar;

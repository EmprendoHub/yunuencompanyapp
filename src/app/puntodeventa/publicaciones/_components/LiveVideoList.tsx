...
const LiveVideoList = ({ data }: { data: string; pagingData?: string }) => {
    ...
    <div className="flex items-center justify-center">
      <div className="w-[80%]">
        <div className="bg-card  p-3 rounded-md h-screen overflow-y-auto">
          <h3 className=" font-bold text-2xl">LIVES</h3>
            ...
            {lives.map((live: any, i: number) => (
              <div key={i} className="flex flex-col">
                <p className="text-xs text-gray-600">
                  {" "}
                  {formatReadableDate(live.creation_time)}
                </p>
                <Link
                  className="text-blue-500"
                  href={`/puntodeventa/live/${live.post_id}`}
                >
                  ...
                  <Button className="mt-5 text-white">Ver Comentarios</Button>
                </Link>

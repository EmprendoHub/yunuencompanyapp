import { getAllPOSOExpenses } from "@/app/_actions";
import Link from "next/link";
import { FiChevronLeft, FiChevronRight } from "react-icons/fi";
import POSExpenses from "./_componets/POSExpenses";

const POSExpensesPage = async ({ searchParams }: { searchParams: any }) => {
  const urlParams = {
  ...
  const data = await getAllPOSOExpenses(searchQuery);
  const expenses = JSON.parse(data.expenses);
  const filteredExpensesCount = data?.itemCount;
...
export default POSExpensesPage;
